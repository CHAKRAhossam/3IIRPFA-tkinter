...
class manag:
    ...
    def open_postform(app):
        # This function will destroy the current CTk window and open a new script.
        app.destroy()
        os.system('python C:\\Users\\Dell\\OneDrive\\Desktop\\finalpfa\\finalpfa\\pfa\\post_form.py')
    ...
    def fetch_data(self):
        ...
        if self.conn is not None and self.conn.is_connected():
            try:
                ...
                cursor.execute("SELECT position_id, position_title FROM pfa_db.positions")
    ...
    def populate_table(self, data):
        ...
        # Add headers
        headers = [["Position ID", "Position Name", "Action"]]
    ...
    def delete_row(self, row_id):
        ...
        if self.conn is not None and self.conn.is_connected():
            try:
                cursor = self.conn.cursor()
                cursor.execute("DELETE FROM pfa_db.positions WHERE position_id = %s", (row_id,))
                self.conn.commit()
                cursor.close()
    ...
    def setup_ui(self):
        ...
        CTkLabel(master=title_frame, text="Positions", font=("Arial Black", 25), text_color="#87CEEB").pack(anchor="nw", side="left")
        ...
        CTkButton(master=search_container, text="ADD", font=("Arial Black", 15), text_color="#FCEDDA", fg_color="#87CEEB",
                hover_color="#0463CA" , command=lambda: manag.open_postform(app)).pack(anchor="ne", side="left", padx=(13, 0), pady=15)

        CTkLabel(master=search_container, text="ONLY ADMIN", font=("Arial Black", 15), text_color="#87CEEB").pack(anchor="ne", side="right", padx=(13, 13), pady=15)
